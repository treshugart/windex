  ...
  Windex.prototype = {
    ...
    request: function(url, data) {
      ...
      request.onreadystatechange = function() {
        ...
        if (typeof headers['Content-Type'] === 'string' && typeof that.parsers[headers['Content-Type']] === 'function') {
          parser = that.parsers[headers['Content-Type']];
        } else if (typeof that.headers.Accept === 'string' && typeof that.parsers[that.headers.Accept] === 'function') {
          parser = that.parsers[that.headers.Accept];
        }

        if (parser) {
          try {
            deferred.resolve(parser(response));
          } catch (e) {
            deferred.reject(new Error('Cannot parse response from "' + url + '" with message: ' + e));
          }
        }
      };
